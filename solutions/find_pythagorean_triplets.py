...
def find_primitive_pythagorean_triplets(n: int) -> List[Tuple[int, int, int]]:
    ...
    triplets = []
    for a in range(1, n + 1):  # Iterate through possible values of a
        for b in range(
            a, n + 1
        ):  # Iterate through possible values of b (starting from a to avoid duplicates)
            c_squared = a**2 + b**2  # Calculate c squared
            c = int(
                c_squared**0.5
            )  # Calculate c by taking the square root of c squared
            if (
                c**2 == c_squared and c <= n
            ):  # Check if c is a perfect square and within the range
                if (
                    gcd(a, b) == 1 and gcd(b, c) == 1 and gcd(a, c) == 1
                ):  # Check if (a, b, c) is a primitive triplet
